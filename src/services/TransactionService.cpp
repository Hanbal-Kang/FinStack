...
bool TransactionService::addTransaction(const Transaction& t) {
    ...
    q.addBindValue(t.get_user_id()); // should be account_id ideally
    q.addBindValue(t.get_category());
...
std::vector<Transaction> TransactionService::getAllByUser(int userId) {
    ...
    while (q.next()) {
        ...
        t.set_type(q.value(3).toString() == "income" ? Transaction::Income : Transaction::Expense);
