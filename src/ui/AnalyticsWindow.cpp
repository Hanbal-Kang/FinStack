...
void AnalyticsWindow::setupUI()
{
    ...
    // bottom: pie chart left, breakdown right
...
void AnalyticsWindow::setupSummaryCards()
{
    ...
    m_highestCatLabel = new QLabel("—",  this);
...
void AnalyticsWindow::loadTransactions()
{
    ...
    // get every transaction for this user from DB
    ...
    if (m_currentPeriod == "This Week")
        ...
        startDate = today.addDays(-30); // This Month

    // keep only transactions within the date range
...
void AnalyticsWindow::updateBreakdownPanel()
{
    ...
    // sort into a list: highest amount first (bubble sort)
